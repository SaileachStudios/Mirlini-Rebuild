...
public static class ProductionLevelImporter {
    ...
    public static AuditedLevels ReadSource() {
        var result=JsonUtility.FromJson<AuditedLevels>(File.ReadAllText(Intermediate));
    ...
    public static void Import(bool write) {
        ...
        try {
            foreach(var source in ReadSource().levels) normalized.Add(Normalize(source,profile));
            ...
            string report="| Level | Solid | Reveal | Start | Goal | Ring | Features | Normalization |\n|---|---:|---:|---|---|---|---|---|\n";
            ...
            foreach(var level in normalized) {
                ...
                report+=$"| {level.MigrationLevelNumber} | {level.Walls.Count(x=>x==WallState.Solid)} | {level.Walls.Count(x=>x==WallState.RevealOnCollision)} | {level.MarbleStartPosition} | {level.HolePosition} | {(level.Unlock.Enabled?level.Unlock.RingPosition.ToString():"—")} | {(level.Unlock.Enabled?"Unlock ":"")}{(level.Dark!=null?"Dark ":"")}{(level.Walls.Contains(WallState.RevealOnCollision)?"Reveal":"")} | {level.MigrationNotes} |\n";
                if(!write) continue;
            ...
            if(write) {
                ...
                AssetDatabase.SaveAssets();File.WriteAllText("Docs/Phase2/Migration.md",report);
            }
    ...
    public static void PlayRepresentatives() {
        ...
        serialized.FindProperty("campaign").objectReferenceValue=AssetDatabase.LoadAssetAtPath<LevelCampaign>(CampaignPath);
